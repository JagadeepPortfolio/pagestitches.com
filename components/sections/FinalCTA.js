"use client";
import { motion } from "framer-motion";
import SpringReveal from "@/components/ui/SpringReveal";
import MagneticButton from "@/components/ui/MagneticButton";
import { spring } from "@/lib/animations";

export default function FinalCTA() {
  return (
    <section className="section-ps bg-navy relative overflow-hidden">
      {/* Gold radial glow */}
      <div
        aria-hidden="true"
        className="absolute inset-0 pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at 50% 50%, rgba(201,168,76,0.08) 0%, transparent 60%)",
        }}
      />

      {/* Ghost text */}
      <div
        aria-hidden="true"
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-extrabold text-[clamp(5rem,14vw,12rem)] text-white/[0.02] whitespace-nowrap pointer-events-none select-none"
      >
        STITCH WEBSITE
      </div>

      <div className="container-ps relative z-10 text-center">
        <SpringReveal>
          <p className="sec-label justify-center mb-6 flex items-center gap-3">
            <span className="w-7 h-[1px] bg-gold" />
            <span>Ready?</span>
          </p>
        </SpringReveal>

        <SpringReveal>
          <h2 className="text-section font-extrabold text-cream mb-4 max-w-2xl mx-auto">
            Your competitors already have websites.
            <br />
            <span className="text-gold font-accent italic font-light">Let&apos;s build yours.</span>
          </h2>
        </SpringReveal>

        <SpringReveal>
          <p className="text-sub text-cream-dim max-w-lg mx-auto mb-10">
            WhatsApp us right now — we reply within 2 hours during business hours. Your website could be live by this weekend.
          </p>
        </SpringReveal>

        <SpringReveal>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <MagneticButton
              href="https://wa.me/?text=Hi%2C%20I%27m%20interested%20in%20a%20website%20for%20my%20business"
              className="btn-gold text-base px-10 py-4"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" className="shrink-0">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              Get Your Website Now
            </MagneticButton>

            <MagneticButton href="/contact" className="btn-ghost text-base px-8 py-4">
              Contact Us →
            </MagneticButton>
          </div>
        </SpringReveal>

        {/* Trust line */}
        <motion.p
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ ...spring.gentle, delay: 0.6 }}
          className="mt-10 text-[13px] text-cream-faint"
        >
          48-hour delivery · Starting ₹5,999 · 50-point quality audit · Free hosting included
        </motion.p>
      </div>
    </section>
  );
}
